...
float pid_incremental_calculate(pid_incremental_t* pid, float current_value)
{
    ...
    // 比例项
    kp_out = pid_out_clamp(pid->kp * (error - pid->last_error), pid->max_out_kp);   // Kp[e(k)-e(k-1)]
    ...
    if (fabs(error) < 1.0f) {
        ...
        // 误差较大时限制积分作用
        ki_out = pid_out_clamp(pid->ki * error, pid->max_out_ki * 0.3f);
    } else {
        // 正常范围内进行积分
        ki_out = pid_out_clamp(pid->ki * error, pid->max_out_ki);
    }
    
    // 微分项
    kd_out = pid_out_clamp(pid->kd * (error - 2 * pid->last_error + pid->prev_error), pid->max_out_kd);
    
    // 计算总输出
    float raw_output = kp_out + ki_out + kd_out;
    output = pid_out_clamp(raw_output, pid->max_out_total);
