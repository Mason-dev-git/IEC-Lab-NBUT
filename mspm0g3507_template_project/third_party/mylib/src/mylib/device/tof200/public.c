...
{
    uint16 i = 0, j = 0;
    for (i = 0, j = 0; i < len; i++, j += 2) { hex_to_ascii(p_data[i], &p_ascii[j]); }
}
...
{
    uint16 i = 0, j = 0;
    for (i = 0, j = 0; i < len; i++, j += 2) { p_data[i] = ascii_to_hex(&p_data[j]); }
}
...
{
    uint16 cksum = 0;
    while (len--) { cksum = a_crc16Table[((cksum >> 8) ^ *p_buff++) & 0xFF] ^ (cksum << 8); }
    return cksum;
}
...
uint8 check_timeout(uint32 start, uint32 now, uint32 invt)
{
    ...
    {
        if ((now >= (uint32)(start + invt)) || (now < start)) { return 1; }
    }
    else   //溢出
    {
        if ((now < start) && (now >= (uint32)(start + invt))) { return 1; }
    }
    return 0;
}
...
{
    if (0xFFFF == p_count[0]) { p_count[0] = 1; }
    else {
        p_count[0]++;
    }
}
...
{
    if (0xFFFFFFFF == p_count[0]) { p_count[0] = 1; }
    else {
        p_count[0]++;
    }
}
...
uint8 bit_lsb_num(uint32 data)
{
    uint8 i = 0;
    for (i = 0; i < 32; i++) {
        if (((uint32)1 << i) == data) { break; }
    }
    return i;
}

/***********************************************************************************************
 *函数名: uint8 lookup_table_index_uint32(const uint32 arr[], uint16 arrLen, uint32 target, uint16
 **pLowIndex, uint16 *pHighIndex) 功能描述: 在数组中查找指定目标下标 特别说明: 函数参数: 函数返回值:
 *修改记录:
 ***********************************************************************************************/
uint8 lookup_table_index_uint32(const uint32 arr[], uint16 arrLen, uint32 target, uint16* pLowIndex,
                                uint16* pHighIndex)
{
    uint16 mid, start, end;
    // 判断输入参数有效性
    if (arr == NULL || arrLen == 0 || pLowIndex == NULL || pHighIndex == NULL) { return 0; }
    start = 0;
    end   = arrLen - 1;
    // 判断目标在界外
    if (target < arr[start] || target > arr[end]) { return 0; }
    if (target == arr[start]) {
        *pLowIndex  = start;
        *pHighIndex = start;
    }
...
uint32 calc_resistor_up(uint32 ResUp, uint32 ResDown, uint16 vt, uint16 Vref)
{
    uint32 RVal;
    // 防止公式计算溢出
    if (vt >= Vref || !vt) { return 0; }
    //计算电阻值
    RVal = (uint32)(Vref - vt) * ResDown;
    RVal /= vt;

    if (RVal > ResUp) { RVal -= ResUp; }
    else {
        RVal = 0;
    }
    return RVal;
...
uint32 calc_resistor_down(uint32 ResUp, uint32 ResDown, uint16 vt, uint16 Vref)
{
    ...
    //防止公式计算溢出
    if (vt >= Vref) { vt = Vref - 1; }
    //计算电阻值
    div1 = ResUp * ((uint32)vt);
    div2 = Vref - vt;
    RVal = (div1 + (div2 >> 1)) / div2;
    //减去下拉保护电阻阻值
    if (RVal >= ResDown) { RVal = RVal - ResDown; }
    else {
        RVal = 0;
    }
    return RVal;
...
uint32 calc_resistor_para(uint32 ParRes, uint32 SrcRes)
{
    ...
    if (SrcRes <= ParRes) { return (uint32)-1; }
    else {
        DstRes = ((uint64)ParRes * SrcRes + (SrcRes - ParRes) / 2) / (SrcRes - ParRes);
        if (DstRes >= (uint32)-1) { return (uint32)-1; }
        else {
...
void unixtime_to_localtime(uint32 unixTime, LOCAL_TIME* p_localTime)
{
    ...
    if ((year % 4 == 0 && year % 100 != 0) || (year % 400 == 0 && year % 3200 != 0) ||
        (year % 172800 == 0)) {
        while (temp >= a_ldayPerMon[month]) { temp -= a_ldayPerMon[month++]; }
    }
    else {
        while (temp >= a_dayPerMon[month]) { temp -= a_dayPerMon[month++]; }
    }
...
void localtime_to_unixtime(LOCAL_TIME* p_localTime, uint32* p_unixTime)
{
    ...
    if ((p_localTime->year % 4 == 0 && p_localTime->year % 100 != 0) ||
        ...
        for (month = 0; month < p_localTime->month; month++) { temp += a_ldayPerMon[month]; }
    }
    else {
        for (month = 0; month < p_localTime->month; month++) { temp += a_dayPerMon[month]; }
    }
