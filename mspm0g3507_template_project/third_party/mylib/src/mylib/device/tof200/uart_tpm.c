...
static uint16 uart_read_isr(uint8 channel, uint8* p_buff, uint16 maxLen)
{
    ...
    for (wrIndex = uartTpmConfig.p_aspConfig[channel].rxWrIndex;
         ...
         uartTpmConfig.p_aspConfig[channel].rxRdIndex =
             (uartTpmConfig.p_aspConfig[channel].rxRdIndex + 1) %
             uartTpmConfig.p_aspConfig[channel].isrRxLen,
        i++,
        maxLen--) {
        p_buff[i] = uartTpmConfig.p_aspConfig[channel]
                        .p_isrRx[uartTpmConfig.p_aspConfig[channel].rxRdIndex];
    }
    return i;
}
...
void uart_tpm_init(UART_TPM_CONFIG* p_Config)
{
    ...
    if (NULL != uartTpmConfig.pfun_mcuUartInit) { uartTpmConfig.pfun_mcuUartInit(); }
}
...
void uart_tpm_main(void)
{
    ...
    uint8  i     = 0;
    for (i = 0; i < uartTpmConfig.aspConfigNum; i++) {
        if (tim_check_timenow(uartTpmConfig.p_aspConfig[i].lastRxTimestamp,
                              uartTpmConfig.framTimeInvt)) {
            ...
            if (rxLen > 2) {
                ...
                if (crc16 == crc16_calculate_modbus(uartTpmConfig.p_buff, rxLen - 2)) {
                    if (NULL != uartTpmConfig.p_aspConfig[i].pfun_recvCallback) {
                        uartTpmConfig.p_aspConfig[i].pfun_recvCallback(uartTpmConfig.p_buff,
                                                                       rxLen - 2);
                    }
                }
            }
...
{
    if (channel >= uartTpmConfig.aspConfigNum) { return 1; }
    if (uartTpmConfig.p_aspConfig[channel].rxRdIndex ==
        ((uartTpmConfig.p_aspConfig[channel].rxWrIndex + 1) %
         uartTpmConfig.p_aspConfig[channel].isrRxLen))   //队列已满
    {
        ...
        uartTpmConfig.p_aspConfig[channel].p_isrRx[uartTpmConfig.p_aspConfig[channel].rxWrIndex] =
            data;
        uartTpmConfig.p_aspConfig[channel].rxWrIndex =
            (uartTpmConfig.p_aspConfig[channel].rxWrIndex + 1) %
            uartTpmConfig.p_aspConfig[channel].isrRxLen;
    }
...
{
    uint16 crc16 = 0;
    if (channel >= uartTpmConfig.aspConfigNum || NULL == p_data || 0 == len ||
    ...
    if (NULL != uartTpmConfig.pfun_mcuUartTx) {
        return uartTpmConfig.pfun_mcuUartTx(
            uartTpmConfig.p_aspConfig[channel].uartId, uartTpmConfig.p_buff, len + 2);
    }
    return 1;
}
