...
void tim_init(TIM_CONFIG* p_config)
{
    ...
    if (NULL != timConfig.pfun_mcuTimInit) { timConfig.pfun_mcuTimInit(timConfig.intvTime); }
}
...
{
    if (NULL != timConfig.pfun_mcuTimDelayus) { timConfig.pfun_mcuTimDelayus(us); }
}
...
void tim_delayms(uint32 ms)
{
    ...
    while (0 == tim_check_timeout(count, tim_get_count(), ms)) {
        if (NULL != timConfig.pfun_mcuKiDog) { timConfig.pfun_mcuKiDog(); }
    }
}
