 ...
 */
#include <device/uart_servo/fashion_star_uart_servo.h>
#include <math.h>
...
FSUS_STATUS FSUS_IsValidResponsePackage(PackageTypeDef* pkg)
{
    ...
    // 判断控制指令是否有效 指令范围超出
    if (pkg->cmdId > FSUS_CMD_NUM) { return FSUS_STATUS_UNKOWN_CMD_ID; }
    // 参数的size大于FSUS_PACK_RESPONSE_MAX_SIZE里面的限制
    if (pkg->size > (FSUS_PACK_RESPONSE_MAX_SIZE - 5)) { return FSUS_STATUS_SIZE_TOO_BIG; }
    // 校验和不匹配
    if (FSUS_CalcChecksum(pkg) != pkg->checksum) { return FSUS_STATUS_CHECKSUM_ERROR; }
    // 数据有效
...
// 构造发送数据帧
void FSUS_SendPackage(usart_data_t* usart, uint8_t cmdId, uint8_t size, uint8_t* content)
{
    // 申请内存
    // DEBUG_INFO("[Package] malloc for pkg\r\n");
    PackageTypeDef pkg;
    ...
    pkg.size = size;
    // 逐一拷贝数组里面的内容
    for (int i = 0; i < size; i++) { pkg.content[i] = content[i]; }
    // 将pkg发送到发送缓冲区sendBuf里面
    FSUS_Package2RingBuffer(&pkg, usart->sendBuf);
    // 通过串口将数据发送出去
    usart_send_all(usart);
}

// 接收数据帧 (在接收的时候动态的申请内存)
FSUS_STATUS FSUS_RecvPackage(usart_data_t* usart, PackageTypeDef* pkg)
{
    ...
    // 倒计时开始
    systick_countdown_begin(FSUS_TIMEOUT_MS);

    // 如果没有超时
    while (!systick_countdown_is_timeout()) {
        ...
        if (pkg->status & FSUS_RECV_FLAG_CONTENT) {
            ...
            if (header == 0) {
                ...
                // 检查第二个字节是否正确
                if (header != FSUS_PACK_RESPONSE_HEADER) { header = 0; }
                else {
                    pkg->header = header;
                    // 帧头接收成功
...
FSUS_STATUS FSUS_Ping(usart_data_t* usart, uint8_t servo_id)
{
    uint8_t statusCode;    // 状态码
    uint8_t ehcoServoId;   // PING得到的舵机ID
    // DEBUG_INFO("[PING]Send Ping Package\r\n");
    // 发送请求包
...
// 重置舵机的用户资料
FSUS_STATUS FSUS_ResetUserData(usart_data_t* usart, uint8_t servo_id)
{
    ...
    if (statusCode == FSUS_STATUS_SUCCESS) {
        ...
        if (result == 1) { return FSUS_STATUS_SUCCESS; }
        else {
...
// 读取数据
FSUS_STATUS FSUS_ReadData(usart_data_t* usart, uint8_t servo_id, uint8_t address, uint8_t* value,
                          uint8_t* size)
{
    FSUS_STATUS statusCode;
    // 构造content
    ...
    if (statusCode == FSUS_STATUS_SUCCESS) {
        ...
        *size = pkg.size - 2;   // content的长度减去servo_id跟address的长度
        // 数据拷贝
        for (int i = 0; i < *size; i++) { value[i] = pkg.content[i + 2]; }
    }
    return statusCode;
}

// 写入数据
FSUS_STATUS FSUS_WriteData(usart_data_t* usart, uint8_t servo_id, uint8_t address, uint8_t* value,
                           uint8_t size)
{
    FSUS_STATUS statusCode;
    // 构造content
    ...
    buffer[1] = address;
    // 拷贝数据
    for (int i = 0; i < size; i++) { buffer[i + 2] = value[i]; }
    // 发送请求数据
    ...
    if (statusCode == FSUS_STATUS_SUCCESS) {
        ...
        if (result == 1) { statusCode = FSUS_STATUS_SUCCESS; }
        else {
 ...
 * speed单位 °/s
 */
FSUS_STATUS FSUS_WheelMove(usart_data_t* usart, uint8_t servo_id, uint8_t method, uint16_t speed,
                           uint16_t value)
{
    // 创建环形缓冲队列
...
// 轮转模式, 舵机停止转动
FSUS_STATUS FSUS_WheelStop(usart_data_t* usart, uint8_t servo_id)
{
    uint8_t  method = 0x00;
    uint16_t speed  = 0;
    uint16_t value  = 0;
...
// 轮转模式 不停旋转
FSUS_STATUS FSUS_WheelKeepMove(usart_data_t* usart, uint8_t servo_id, uint8_t is_cw, uint16_t speed)
{
...
// 轮转模式 按照特定的速度旋转特定的时间
FSUS_STATUS FSUS_WheelMoveTime(usart_data_t* usart, uint8_t servo_id, uint8_t is_cw, uint16_t speed,
                               uint16_t nTime)
{
...
// 轮转模式 旋转特定的圈数
FSUS_STATUS FSUS_WheelMoveNCircle(usart_data_t* usart, uint8_t servo_id, uint8_t is_cw,
                                  uint16_t speed, uint16_t nCircle)
{
...
// @power 舵机执行功率 单位mW
//        若power=0或者大于保护值
FSUS_STATUS FSUS_SetServoAngle(usart_data_t* usart, uint8_t servo_id, float angle,
                               uint16_t interval, uint16_t power, uint8_t wait)
{
    // 创建环形缓冲队列
    ...
    if (wait) { return FSUS_Wait(usart, servo_id, angle, 0); }
    else {
        return FSUS_STATUS_SUCCESS;
    }
}
...
/* 设置舵机的角度(指定周期) */
FSUS_STATUS FSUS_SetServoAngleByInterval(usart_data_t* usart, uint8_t servo_id, float angle,
                                         uint16_t interval, uint16_t t_acc, uint16_t t_dec,
                                         uint16_t power, uint8_t wait)
{
    // 创建环形缓冲队列
...
/* 设置舵机的角度(指定转速) */
FSUS_STATUS FSUS_SetServoAngleByVelocity(usart_data_t* usart, uint8_t servo_id, float angle,
                                         float velocity, uint16_t t_acc, uint16_t t_dec,
                                         uint16_t power, uint8_t wait)
{
    // 创建环形缓冲队列
    const uint8_t     size = 11;
...
FSUS_STATUS FSUS_QueryServoAngle(usart_data_t* usart, uint8_t servo_id, float* angle)
{
...
/* 设置舵机的角度(多圈模式) */
FSUS_STATUS FSUS_SetServoAngleMTurn(usart_data_t* usart, uint8_t servo_id, float angle,
                                    uint32_t interval, uint16_t power, uint8_t wait)
{
    ...
    if (interval > 4096000) { angle = 4096000; }
    // 协议打包
    RingBuffer_WriteByte(&ringBuf, servo_id);
    ...
    if (wait) { return FSUS_Wait(usart, servo_id, angle, 1); }
    else {
        return FSUS_STATUS_SUCCESS;
    }
}
...
/* 设置舵机的角度(多圈模式, 指定周期) */
FSUS_STATUS FSUS_SetServoAngleMTurnByInterval(usart_data_t* usart, uint8_t servo_id, float angle,
                                              uint32_t interval, uint16_t t_acc, uint16_t t_dec,
                                              uint16_t power, uint8_t wait)
{
    // 创建环形缓冲队列
    ...
    if (interval > 4096000) { interval = 4096000; }
    if (t_acc < 20) { t_acc = 20; }
    if (t_dec < 20) { t_dec = 20; }
    // 协议打包
    RingBuffer_WriteByte(&ringBuf, servo_id);
...
/* 设置舵机的角度(多圈模式, 指定转速) */
FSUS_STATUS FSUS_SetServoAngleMTurnByVelocity(usart_data_t* usart, uint8_t servo_id, float angle,
                                              float velocity, uint16_t t_acc, uint16_t t_dec,
                                              uint16_t power, uint8_t wait)
{
    // 创建环形缓冲队列
...
/* 查询舵机的角度(多圈模式) */
FSUS_STATUS FSUS_QueryServoAngleMTurn(usart_data_t* usart, uint8_t servo_id, float* angle)
{
    // 创建环形缓冲队列
    const uint8_t size = 1;   // 请求包content的长度
    uint8_t       ehcoServoId;
    ...
    if (statusCode == FSUS_STATUS_SUCCESS) {
        ...
        // 提取舵机角度
        echoAngle = (int32_t)(pkg.content[1] | (pkg.content[2] << 8) | (pkg.content[3] << 16) |
                              (pkg.content[4] << 24));
        *angle    = (float)(echoAngle / 10.0);
    }
    return statusCode;
...
/* 舵机阻尼模式 */
FSUS_STATUS FSUS_DampingMode(usart_data_t* usart, uint8_t servo_id, uint16_t power)
{
...
/* 等待电机旋转到特定的位置 */
FSUS_STATUS FSUS_Wait(usart_data_t* usart, uint8_t servo_id, float target_angle, uint8_t is_mturn)
{
    ...
    // 循环等待
    while (1) {
        if (is_mturn) { FSUS_QueryServoAngleMTurn(usart, servo_id, &angle_read); }
        else {
            FSUS_QueryServoAngle(usart, servo_id, &angle_read);
        }

        angle_error = fabsf(target_angle - angle_read);
        // DEBUG_INFO("status:%d, angle:%.1f ,angle_error: %.1f\r\n", status, angle_read,
        // angle_error);
        if (angle_error <= FSUS_ANGLE_DEADAREA) { return FSUS_STATUS_SUCCESS; }

        // 超时判断机制
        count += 1;
        if (count >= FSUS_WAIT_COUNT_MAX) { return FSUS_STATUS_FAIL; }
        // 延时100ms
