...
void OLED_ShowImage(uint8_t X, uint8_t Y0, uint8_t Width, uint8_t Height, const uint8_t* Image)
{
    ...
    if (Y < 0)   // 如果Y小于0, 则需打印图片位于屏幕内的部分;
    {
        ...
        l = (-Y - 1) % 8 + 1;   // Byte位移基	//(8 + Y % 8)%8;//		OLED_ShowNum(0, 56, l, 3,
                                // 6);
        Y = 0;                  // 图片仍然从屏幕Y0开始打印
        if ((int8_t)Height < 1) { return; }   // 需打印的图片高小于1时,说明不需要打印;
    }
    else if (Y > 63) {
        return;
    }   // 如果Y大于63则打印无意义,故无需打印
 ...
 * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
uint8_t OLED_ShowStringArea(uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height, char* String,
                            uint8_t FontSize)   // 区域打印打印;
{
    uint8_t i = 0, len = 0;
    while (String[i] != '\0')   // 遍历字符串的每个字符
    {
        if (String[i] == '\n')   // 兼容换行符
        {
            ...
            if (Height < ((FontSize == 8) ? 16 : 8)) { break; }
