...
// 发送字节
void usart_send_byte(UART_HandleTypeDef* huart, uint8_t ch)
{
    HAL_UART_Transmit(huart, &ch, 1, 1000);
}

// 发送字节数组
void usart_send_byte_array(UART_HandleTypeDef* huart, uint8_t* byte_array, uint16_t size)
{
    HAL_UART_Transmit(huart, byte_array, size, 1000);
}
...
// 将串口发送缓冲区的内容全部发出去
void usart_send_all(usart_data_t* usart)
{
    uint8_t value;
    while (RingBuffer_GetByteUsed(usart->sendBuf)) {
        value = RingBuffer_Pop(usart->sendBuf);
        // DEBUG_INFO("Usart_SendAll pop: %d", value);
        usart_send_byte(usart->huart, value);
    }
}
...
//        USART_HardwareFlowControl_None;//无硬件数据流控制 USART_InitStructure.USART_Mode =
