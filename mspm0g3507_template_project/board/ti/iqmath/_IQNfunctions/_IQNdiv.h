...
#endif
__STATIC_INLINE int_fast32_t __IQNdiv(int_fast32_t iqNInput1, int_fast32_t iqNInput2,
                                      const uint8_t type, const int8_t q_value)
{
    ...
    if (type == TYPE_DEFAULT) {
        ...
        if (iqNInput2 <= 0) {
            ...
            if (iqNInput2 == 0) { return INT32_MAX; }
            else {
                ui8Sign   = 1;
                iqNInput2 = -iqNInput2;
            }
     ...
     */
    if (q_value < 31) { uiiqNInput1 >>= (31 - q_value - 1); }
    else {
        uiiqNInput1 <<= 1;
    }
    ...
    if (type == TYPE_DEFAULT) {
        if (uiqNResult > INT32_MAX) {
            ...
            if (ui8Sign) { return -(int_fast32_t)uiqNResult; }
            else {
...
// TODO: unless we find a different use for it, or we are intending to keep same params as RTS
// function, I see no use for TYPE here.
