...
#    endif
__STATIC_INLINE int_fast32_t __IQNatan2(int_fast32_t iqNInputY, int_fast32_t iqNInputX,
                                        const uint8_t type, const int8_t q_value)
{
    ...
    if (type == TYPE_PU) {
        ...
        if (q_value < 29) { iq29Result += (uint_fast32_t)1 << (28 - q_value); }
    ...
    if (ui8Status & 1) { return -iqNResult; }
    else {
        return iqNResult;
    }
}
                                        ...
                                        const uint8_t type, const int8_t q_value)
{
    ...
    if (q_value < 28) { res = res >> (28 - q_value); }
    else {
        res = res << (q_value - 28);
    }
