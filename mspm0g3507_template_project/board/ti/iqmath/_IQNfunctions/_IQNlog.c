...
#endif
__STATIC_INLINE int_fast32_t __IQNlog(int_fast32_t iqNInput, const int_fast32_t iqNMin,
                                      const int8_t q_value)
{
     ...
     * unsigned data type.
     */
    if (i16Exp > 0) { iqNResult += __mpyf_ul(iq31_ln2, ((int_fast32_t)i16Exp << q_value)); }
    else {
