...
#endif
__STATIC_INLINE int_fast32_t __IQNsqrt(int_fast32_t iqNInputX, int_fast32_t iqNInputY,
                                       const int8_t q_value, const int8_t type)
{
    uint8_t       ui8Index;
    uint8_t       ui8Loops;
    ...
    if (type == TYPE_MAG || type == TYPE_IMAG) {
        ...
        if ((32 - q_value) % 2 == 1) {
            ...
            if (type == TYPE_SQRT) { i16Exponent = ((32 - q_value) - 1) >> 1; }
            ...
            /* start with positive exponent for sqrt */
            if (type == TYPE_SQRT) { i16Exponent = (32 - q_value) >> 1; }
            /* start with negative exponent for isqrt */
            else {
    ...
    if (type == TYPE_SQRT || type == TYPE_MAG) {
        ...
        if (type == TYPE_MAG) {
            if (i16Exponent > 0) { return 0x7fffffff; }
        }
