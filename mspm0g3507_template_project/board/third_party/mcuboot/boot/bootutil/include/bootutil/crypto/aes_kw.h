...
#if (defined(MCUBOOT_USE_MBED_TLS) + defined(MCUBOOT_USE_TINYCRYPT)) != 1
...
static inline int bootutil_aes_kw_set_unwrap_key(bootutil_aes_kw_context* ctx, const uint8_t* k,
                                                 uint32_t klen)
{
    return mbedtls_nist_kw_setkey(ctx, MBEDTLS_CIPHER_ID_AES, k, klen * 8, 0);
}

static inline int bootutil_aes_kw_unwrap(bootutil_aes_kw_context* ctx, const uint8_t* wrapped_key,
                                         uint32_t wrapped_key_len, uint8_t* key, uint32_t key_len)
{
    size_t olen;
    return mbedtls_nist_kw_unwrap(
        ctx, MBEDTLS_KW_MODE_KW, wrapped_key, wrapped_key_len, key, &olen, key_len);
}
#endif /* MCUBOOT_USE_MBED_TLS */
                                                 ...
                                                 uint32_t klen)
{
    ...
    if (rc != TC_CRYPTO_SUCCESS) { return -1; }
    return 0;
}
 ...
 */
static int bootutil_aes_kw_unwrap(bootutil_aes_kw_context* ctx, const uint8_t* wrapped_key,
                                  uint32_t wrapped_key_len, uint8_t* key, uint32_t key_len)
{
    uint8_t A[8];
    uint8_t B[16];
    ...
    if (wrapped_key_len != 24 || key_len != 16) { return -1; }
    ...
    for (j = 5; j >= 0; j--) {
        for (i = 2; i > 0; i--) {
            ...
            B[7] ^= 2 * j + i;
            if (tc_aes_decrypt((uint8_t*)&B, (uint8_t*)&B, ctx) != TC_CRYPTO_SUCCESS) { return -1; }
            for (k = 0; k < 8; k++) {
                A[k]                   = B[k];
                key[((i - 1) * 8) + k] = B[8 + k];
            }
