...
__attribute__((always_inline)) inline int fih_delay(void)
{
    unsigned char delay;
    int           foo = 0;
    volatile int  rc;
    ...
    for (volatile int i = 0; i < delay; i++) { foo++; }
...
__attribute__((always_inline)) inline int fih_delay_init(void)
{
    return 1;
}
...
{
    if (x.val != (x.msk ^ _fih_mask)) { FIH_PANIC; }
}

/* Convert a fih_int to an int. Validate for tampering. */
__attribute__((always_inline)) inline int fih_int_decode(fih_int x)
{
    fih_int_validate(x);
    return x.val;
}
...
__attribute__((always_inline)) inline fih_int fih_int_encode(int x)
{
...
__attribute__((always_inline)) inline int fih_not_eq(fih_int x, fih_int y)
{
    fih_int_validate(x);
    fih_int_validate(y);
    return (x.val != y.val) && fih_delay() && (x.msk != y.msk);
 ...
 */
__attribute__((always_inline)) inline fih_int fih_int_encode_zero_equality(int x)
{
    if (x) { return FIH_FAILURE; }
    else {
        return FIH_SUCCESS;
    }
}
...
#    define FIH_CALL2(f, ret, l, c, ...)               \
        do {                                           \
            ...
            ret = FIH_FAILURE;                         \
            if (fih_delay()) { ret = f(__VA_ARGS__); } \
 ...
 * called.
 */
#    define FIH_CFI_PRECALL_BLOCK fih_int _fih_cfi_saved_value = fih_cfi_get_and_increment()

#    define FIH_CFI_POSTCALL_BLOCK fih_cfi_validate(_fih_cfi_saved_value)
