...
#define BOOT_NUM_SLOTS 2
     ...
     defined(MCUBOOT_DIRECT_XIP) + defined(MCUBOOT_RAM_LOAD)) > 1
#    error \
        "Please enable only one of MCUBOOT_OVERWRITE_ONLY, MCUBOOT_SWAP_USING_MOVE, MCUBOOT_DIRECT_XIP or MCUBOOT_RAM_LOAD"
#endif

#if !defined(MCUBOOT_OVERWRITE_ONLY) && !defined(MCUBOOT_SWAP_USING_MOVE) && \
    !defined(MCUBOOT_DIRECT_XIP) && !defined(MCUBOOT_RAM_LOAD)
...
#        error \
            "The MCUBOOT_DIRECT_XIP and MCUBOOT_RAM_LOAD mode only supports single-image boot (MCUBOOT_IMAGE_NUMBER=1)."
#    endif
#    ifdef MCUBOOT_ENC_IMAGES
#        error \
            "Image encryption (MCUBOOT_ENC_IMAGES) is not supported when MCUBOOT_DIRECT_XIP or MCUBOOT_RAM_LOAD mode is selected."
#    endif
...
fih_int bootutil_verify_sig(uint8_t* hash, uint32_t hlen, uint8_t* sig, size_t slen,
                            uint8_t key_id);
...
int      boot_read_swap_state(const struct flash_area* fap, struct boot_swap_state* state);
...
int      boot_write_swap_info(const struct flash_area* fap, uint8_t swap_type, uint8_t image_num);
...
int  boot_read_image_header(struct boot_loader_state* state, int slot, struct image_header* out_hdr,
...
#ifdef MCUBOOT_ENC_IMAGES
int boot_write_enc_key(const struct flash_area* fap, uint8_t slot, const struct boot_status* bs);
 ...
 */
bool bootutil_buffer_is_erased(const struct flash_area* area, const void* buffer, size_t len);
...
static inline size_t boot_img_num_sectors(const struct boot_loader_state* state, size_t slot)
{
 ...
 */
static inline uint32_t boot_img_slot_off(struct boot_loader_state* state, size_t slot)
{
...
static inline size_t boot_img_sector_size(const struct boot_loader_state* state, size_t slot,
                                          size_t sector)
{
 ...
 * device.
 */
static inline uint32_t boot_img_sector_off(const struct boot_loader_state* state, size_t slot,
                                           size_t sector)
{
    return BOOT_IMG(state, slot).sectors[sector].fa_off - BOOT_IMG(state, slot).sectors[0].fa_off;
}
...
static inline uint32_t boot_img_sector_off(const struct boot_loader_state* state, size_t slot,
                                           size_t sector)
{
    return BOOT_IMG(state, slot).sectors[sector].fs_off - BOOT_IMG(state, slot).sectors[0].fs_off;
}
...
#    define LOAD_IMAGE_DATA(hdr, fap, start, output, size) \
        (memcpy((output), (void*)((hdr)->ih_load_addr + (start)), (size)), 0)
#else
#    define LOAD_IMAGE_DATA(hdr, fap, start, output, size) \
