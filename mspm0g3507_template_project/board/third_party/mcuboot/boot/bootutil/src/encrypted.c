...
static int bootutil_constant_time_compare(const uint8_t* a, const uint8_t* b, size_t size)
{
    ...
    for (i = 0; i < size; i++) { result |= tempa[i] ^ tempb[i]; }
    return result;
}
#        endif
#    endif
...
#    if defined(MCUBOOT_ENCRYPT_RSA)
static int parse_rsa_enckey(mbedtls_rsa_context* ctx, uint8_t** p, uint8_t* end)
{
    size_t len;
    ...
    if (/* version */
        ...
        /* primes */
        mbedtls_asn1_get_mpi(p, end, &ctx->P) != 0 || mbedtls_asn1_get_mpi(p, end, &ctx->Q) != 0) {
...
#        if !defined(MBEDTLS_RSA_NO_CRT)
    ...
    if (*p < end) {
        ...
        if (mbedtls_rsa_deduce_crt(&ctx->P, &ctx->Q, &ctx->D, &ctx->DP, &ctx->DQ, &ctx->QP) != 0) {
            return -5;
        }
    }
#        endif
    ...
    if (mbedtls_rsa_check_privkey(ctx) != 0) { return -6; }
 ...
 */
static int parse_ec256_enckey(uint8_t** p, uint8_t* end, uint8_t* private_key)
{
    int              rc;
    size_t           len;
    int              version;
    ...
    version = 0;
    if (mbedtls_asn1_get_int(p, end, &version) || version != 0) { return -3; }

    if ((rc = mbedtls_asn1_get_alg(p, end, &alg, &param)) != 0) { return -5; }
    ...
    if ((rc = mbedtls_asn1_get_tag(p, end, &len, MBEDTLS_ASN1_OCTET_STRING)) != 0) { return -8; }
    ...
    version = 0;
    if (mbedtls_asn1_get_int(p, end, &version) || version != 1) { return -10; }

    /* privateKey */

    if ((rc = mbedtls_asn1_get_tag(p, end, &len, MBEDTLS_ASN1_OCTET_STRING)) != 0) { return -11; }

    if (len != NUM_ECC_BYTES) { return -12; }
...
static const uint8_t                                   ec_pubkey_oid[] =
    MBEDTLS_OID_ISO_IDENTIFIED_ORG MBEDTLS_OID_ORG_GOV X25519_OID;

#        define SHARED_KEY_LEN 32
#        define PRIV_KEY_LEN 32

static int parse_x25519_enckey(uint8_t** p, uint8_t* end, uint8_t* private_key)
{
    size_t           len;
    int              version;
    mbedtls_asn1_buf alg;
    ...
    if (mbedtls_asn1_get_alg(p, end, &alg, &param) != 0) { return -4; }
    ...
    if (mbedtls_asn1_get_tag(p, end, &len, MBEDTLS_ASN1_OCTET_STRING) != 0) { return -6; }

    if (mbedtls_asn1_get_tag(p, end, &len, MBEDTLS_ASN1_OCTET_STRING) != 0) { return -7; }

    if (len != PRIV_KEY_LEN) { return -8; }
 ...
 */
static int hkdf(uint8_t* ikm, uint16_t ikm_len, uint8_t* info, uint16_t info_len, uint8_t* okm,
                uint16_t* okm_len)
{
    ...
    if (ikm == NULL || okm == NULL || ikm_len == 0) { return -1; }
...
int boot_enc_set_key(struct enc_key_data* enc_state, uint8_t slot, const struct boot_status* bs)
{
    int rc;
...
#    if defined(MCUBOOT_ENCRYPT_RSA)
    ...
    rc = mbedtls_rsa_rsaes_oaep_decrypt(
...
#    if defined(MCUBOOT_ENCRYPT_EC256) || defined(MCUBOOT_ENCRYPT_X25519)
    ...
    rc = bootutil_aes_ctr_decrypt(
        &aes_ctr, counter, &buf[EC_CIPHERKEY_INDEX], BOOTUTIL_CRYPTO_AES_CTR_KEY_SIZE, 0, enckey);
    if (rc != 0) {
        bootutil_aes_ctr_drop(&aes_ctr);
        return -1;
    }
...
#    endif
    ...
    /* Already loaded... */
    if (enc_state[slot].valid) { return 1; }
...
void boot_encrypt(struct enc_key_data* enc_state, int image_index, const struct flash_area* fap,
...
{
    uint8_t slot;
    for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) { (void)boot_enc_drop(enc_state, slot); }
