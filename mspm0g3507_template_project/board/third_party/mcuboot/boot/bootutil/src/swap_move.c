                           ...
                           struct boot_status* bs)
{
    ...
    if (max_entries < 0) { return BOOT_EBADARGS; }
...
#    endif
    ...
    if (found_idx == -1) { /* no swap status found; nothing to do */
    }
        ...
        bs->state =
            ((found_idx - move_entries) % BOOT_STATUS_SWAP_STATE_COUNT) + BOOT_STATUS_STATE_0;
    }
...
uint32_t boot_status_internal_off(const struct boot_status* bs, int elem_sz)
{
    ...
    idx_sz = elem_sz * ((bs->op == BOOT_STATUS_OP_MOVE) ? BOOT_STATUS_MOVE_STATE_COUNT
                                                        : BOOT_STATUS_SWAP_STATE_COUNT);

    off = ((bs->op == BOOT_STATUS_OP_MOVE)
               ? 0
               : (BOOT_MAX_IMG_SECTORS * BOOT_STATUS_MOVE_STATE_COUNT * elem_sz)) +
          (bs->idx - BOOT_STATUS_IDX_0) * idx_sz + (bs->state - BOOT_STATUS_STATE_0) * elem_sz;
...
int boot_slots_compatible(struct boot_loader_state* state)
{
    ...
    if ((num_sectors_pri != num_sectors_sec) && (num_sectors_pri != (num_sectors_sec + 1))) {
 ...
 */
static void boot_move_sector_up(int idx, uint32_t sz, struct boot_loader_state* state,
...
#    endif
    ...
    /* No fixup required */
    if (bs->swap_type != BOOT_SWAP_TYPE_REVERT || bs->op != BOOT_STATUS_OP_MOVE ||
...
void swap_run(struct boot_loader_state* state, struct boot_status* bs, uint32_t copy_size)
{
    ...
    while (idx <= g_last_idx) {
        if (idx >= bs->idx) { boot_swap_sectors(idx, sector_sz, state, bs, fap_pri, fap_sec); }
        idx++;
    }
