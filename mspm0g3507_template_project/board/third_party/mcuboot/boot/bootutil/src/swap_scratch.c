                           ...
                           struct boot_status* bs)
{
    ...
    for (i = 0; i < max_entries; i++) {
        rc = flash_area_read(fap, off + i * BOOT_WRITE_SZ(state), &status, 1);
        if (rc < 0) { return BOOT_EFLASH; }

        if (bootutil_buffer_is_erased(fap, &status, 1)) {
            if (found && !found_idx) { found_idx = i; }
        }
        else if (!found) {
            found = 1;
        }
...
uint32_t boot_status_internal_off(const struct boot_status* bs, int elem_sz)
{
    ...
    return (bs->idx - BOOT_STATUS_IDX_0) * idx_sz + (bs->state - BOOT_STATUS_STATE_0) * elem_sz;
}
...
#        define BOOT_STATUS_TABLES_COUNT (sizeof boot_status_tables / sizeof boot_status_tables[0])
                              ...
                              struct boot_status* bs)
{
    ...
    if (bs->state == BOOT_STATUS_STATE_1) {
        ...
        rc = boot_copy_region(
            state, fap_primary_slot, fap_secondary_slot, img_off, img_off, copy_sz);
        assert(rc == 0);
    ...
    if (bs->state == BOOT_STATUS_STATE_2) {
        ...
        if (bs->use_scratch) {
            ...
            rc = boot_read_swap_state_by_id(FLASH_AREA_IMAGE_SCRATCH, &swap_state);
            assert(rc == 0);
            ...
            if (swap_state.swap_type != BOOT_SWAP_TYPE_NONE) {
                rc = boot_write_swap_info(fap_primary_slot, swap_state.swap_type, image_index);
                assert(rc == 0);
            }
...
void swap_run(struct boot_loader_state* state, struct boot_status* bs, uint32_t copy_size)
{
    ...
    while (1) {
        ...
        if ((secondary_slot_size < copy_size) || (secondary_slot_size < primary_slot_size)) {
            secondary_slot_size +=
                boot_img_sector_size(state, BOOT_SECONDARY_SLOT, last_idx_secondary_slot);
        }
        if (primary_slot_size >= copy_size && secondary_slot_size >= copy_size &&
