...
static uint32_t boot_write_sz(struct boot_loader_state* state)
{
    uint32_t elem_sz;
...
static int boot_initialize_area(struct boot_loader_state* state, int flash_area)
{
    ...
    if (flash_area == FLASH_AREA_IMAGE_PRIMARY(BOOT_CURR_IMG(state))) {
        rc = flash_area_to_sectors(
            flash_area, &num_sectors, BOOT_IMG(state, BOOT_PRIMARY_SLOT).sectors);
        BOOT_IMG(state, BOOT_PRIMARY_SLOT).num_sectors = (size_t)num_sectors;
    }
    else if (flash_area == FLASH_AREA_IMAGE_SECONDARY(BOOT_CURR_IMG(state))) {
        rc = flash_area_to_sectors(
            flash_area, &num_sectors, BOOT_IMG(state, BOOT_SECONDARY_SLOT).sectors);
...
{
    return (bs->op == BOOT_STATUS_OP_MOVE && bs->idx == BOOT_STATUS_IDX_0 &&
            bs->state == BOOT_STATUS_STATE_0);
}
...
#        endif
    ...
    off        = boot_status_off(fap) + boot_status_internal_off(bs, BOOT_WRITE_SZ(state));
                                ...
                                const struct flash_area* fap, struct boot_status* bs)
{
    ...
    if (MUST_DECRYPT(fap, image_index, hdr)) {
        ...
        if (rc < 0) { FIH_RET(fih_rc); }
        if (rc == 0 && boot_enc_set_key(BOOT_CURR_ENC(state), 1, bs)) { FIH_RET(fih_rc); }
    }
#endif

    FIH_CALL(bootutil_img_validate,
             fih_rc,
             BOOT_CURR_ENC(state),
             image_index,
             hdr,
...
static fih_int split_image_check(struct image_header* app_hdr, const struct flash_area* app_fap,
                                 ...
                                 const struct flash_area* loader_fap)
{
    ...
    FIH_CALL(bootutil_img_validate,
             ...
             0,
             loader_hash);
    if (fih_not_eq(fih_rc, FIH_SUCCESS)) { FIH_RET(fih_rc); }
...
static bool boot_is_header_valid(const struct image_header* hdr, const struct flash_area* fap)
{
    ...
    if (hdr->ih_magic != IMAGE_MAGIC) { return false; }

    if (!boot_u32_safe_add(&size, hdr->ih_img_size, hdr->ih_hdr_size)) { return false; }

    if (size >= fap->fa_size) { return false; }
...
static int boot_check_header_erased(struct boot_loader_state* state, int slot)
{
    ...
    if (!boot_data_is_set_to(erased_val, &hdr->ih_magic, sizeof(hdr->ih_magic))) { return -1; }

    return 0;
}

#if (BOOT_IMAGE_NUMBER > 1) || defined(MCUBOOT_DIRECT_XIP) || defined(MCUBOOT_RAM_LOAD) || \
 ...
 */
static int boot_version_cmp(const struct image_version* ver1, const struct image_version* ver2)
{
    if (ver1->iv_major > ver2->iv_major) { return 1; }
    if (ver1->iv_major < ver2->iv_major) { return -1; }
    /* The major version numbers are equal, continue comparison. */
    if (ver1->iv_minor > ver2->iv_minor) { return 1; }
    if (ver1->iv_minor < ver2->iv_minor) { return -1; }
    /* The minor version numbers are equal, continue comparison. */
    if (ver1->iv_revision > ver2->iv_revision) { return 1; }
    if (ver1->iv_revision < ver2->iv_revision) { return -1; }
 ...
 */
static fih_int boot_validate_slot(struct boot_loader_state* state, int slot, struct boot_status* bs)
{
    const struct flash_area* fap;
    struct image_header*     hdr;
    int                      area_id;
    ...
    hdr = boot_img_hdr(state, slot);
    if (boot_check_header_erased(state, slot) == 0 || (hdr->ih_flags & IMAGE_F_NON_BOOTABLE)) {
         ...
         * is erased.
         */
        if (slot != BOOT_PRIMARY_SLOT) { swap_erase_trailer_sectors(state, fap); }
#endif
 ...
 * @return              0 on success; nonzero on failure.
 */
static int boot_update_security_counter(uint8_t image_index, int slot, struct image_header* hdr)
{
    const struct flash_area* fap = NULL;
    uint32_t                 img_security_cnt;
    int                      rc;

    rc = flash_area_open(flash_area_id_from_multi_image_slot(image_index, slot), &fap);
    if (rc != 0) {
        rc = BOOT_EFLASH;
        goto done;
    }
 ...
 */
static int boot_validated_swap_type(struct boot_loader_state* state, struct boot_status* bs)
{
    int     swap_type;
    fih_int fih_rc = FIH_FAILURE;
    ...
    if (BOOT_IS_UPGRADE(swap_type)) {
        ...
        if (fih_not_eq(fih_rc, FIH_SUCCESS)) {
            if (fih_eq(fih_rc, fih_int_encode(1))) { swap_type = BOOT_SWAP_TYPE_NONE; }
            else {
 ...
 * @return                      0 on success; nonzero on failure.
 */
int boot_copy_region(struct boot_loader_state* state, const struct flash_area* fap_src,
                     const struct flash_area* fap_dst, uint32_t off_src, uint32_t off_dst,
                     uint32_t sz)
{
...
#    endif
    ...
    while (bytes_copied < sz) {
        if (sz - bytes_copied > sizeof buf) { chunk_sz = sizeof buf; }
        else {
...
#        endif
            if (IS_ENCRYPTED(hdr)) {
                ...
                if (off + bytes_copied + chunk_sz > tlv_off) {
                    /* do not decrypt TLVs */
                    if (off + bytes_copied >= tlv_off) { blk_sz = 0; }
                    else {
                        blk_sz = tlv_off - (off + bytes_copied);
                    }
                }
                boot_encrypt(BOOT_CURR_ENC(state),
                             image_index,
                             fap_src,
                             (off + bytes_copied + idx) - hdr->ih_hdr_size,
                             blk_sz,
                             blk_off,
                             &buf[idx]);
            }
...
#        ifdef MCUBOOT_ENC_IMAGES
    if (IS_ENCRYPTED(boot_img_hdr(state, BOOT_SECONDARY_SLOT))) {
        rc = boot_enc_load(BOOT_CURR_ENC(state),
                           image_index,
                           boot_img_hdr(state, BOOT_SECONDARY_SLOT),
                           fap_secondary_slot,
                           bs);

        if (rc < 0) { return BOOT_EBADIMAGE; }
        if (rc == 0 && boot_enc_set_key(BOOT_CURR_ENC(state), 1, bs)) { return BOOT_EBADIMAGE; }
    }
#        endif

    BOOT_LOG_INF("Copying the secondary slot to the primary slot: 0x%zx bytes", size);
...
#        ifdef MCUBOOT_HW_ROLLBACK_PROT
    ...
    rc = boot_update_security_counter(
        BOOT_CURR_IMG(state), BOOT_PRIMARY_SLOT, boot_img_hdr(state, BOOT_SECONDARY_SLOT));
    if (rc != 0) {
...
#        ifdef MCUBOOT_ENC_IMAGES
        for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
            ...
            for (i = 0; i < BOOT_ENC_KEY_SIZE; i++) {
                if (bs->enckey[slot][i] != 0xff) { break; }
            }

            if (i != BOOT_ENC_KEY_SIZE) { boot_enc_set_key(BOOT_CURR_ENC(state), slot, bs); }
        }
#        endif
    }
                                       ...
                                       struct image_dependency*  dep)
{
    ...
    dep_slot    = BOOT_IS_UPGRADE(swap_type) ? BOOT_SECONDARY_SLOT : BOOT_PRIMARY_SLOT;
    ...
    if (rc < 0) {
        ...
        case BOOT_SWAP_TYPE_PERM: BOOT_SWAP_TYPE(state) = BOOT_SWAP_TYPE_NONE; break;
        case BOOT_SWAP_TYPE_NONE: BOOT_SWAP_TYPE(state) = BOOT_SWAP_TYPE_REVERT; break;
        default: break;
        }
    }
    else {
...
static int boot_verify_slot_dependencies(struct boot_loader_state* state, uint32_t slot)
{
    ...
    rc = bootutil_tlv_iter_begin(&it, boot_img_hdr(state, slot), fap, IMAGE_TLV_DEPENDENCY, true);
    if (rc != 0) { goto done; }
...
#    elif defined(MCUBOOT_BOOTSTRAP)
    ...
    if (rc == 0 || fih_not_eq(fih_rc, FIH_SUCCESS)) { rc = boot_copy_image(state, bs); }
    else {
...
#    ifndef MCUBOOT_OVERWRITE_ONLY
    ...
    if (swap_type == BOOT_SWAP_TYPE_REVERT || swap_type == BOOT_SWAP_TYPE_PERM) {
        ...
        if (rc != 0) { BOOT_SWAP_TYPE(state) = swap_type = BOOT_SWAP_TYPE_PANIC; }
    }
...
#        endif /* MCUBOOT_HW_ROLLBACK_PROT */
    ...
    if (BOOT_IS_UPGRADE(swap_type)) {
        ...
        if (rc != 0) { BOOT_SWAP_TYPE(state) = BOOT_SWAP_TYPE_PANIC; }
    }
#    endif /* !MCUBOOT_OVERWRITE_ONLY */
 ...
 */
#    if !defined(MCUBOOT_OVERWRITE_ONLY)
static int boot_complete_partial_swap(struct boot_loader_state* state, struct boot_status* bs)
{
    int rc;
 ...
 */
static void boot_review_image_swap_types(struct boot_loader_state* state, bool aborted_swap)
{
 ...
 */
static void boot_prepare_image_for_update(struct boot_loader_state* state, struct boot_status* bs)
{
    int     rc;
    fih_int fih_rc = FIH_FAILURE;
    ...
    if (rc != 0) {
        ...
        BOOT_LOG_WRN("Failed reading image headers; Image=%u", BOOT_CURR_IMG(state));
        BOOT_SWAP_TYPE(state) = BOOT_SWAP_TYPE_NONE;
        return;
    }
...
#    ifdef MCUBOOT_BOOTSTRAP
            if (BOOT_SWAP_TYPE(state) == BOOT_SWAP_TYPE_NONE) {
                ...
                if (rc == 0 || fih_not_eq(fih_rc, FIH_SUCCESS)) {
                    ...
                    rc =
                        (boot_img_hdr(state, BOOT_SECONDARY_SLOT)->ih_magic == IMAGE_MAGIC) ? 1 : 0;
                    FIH_CALL(boot_validate_slot, fih_rc, state, BOOT_SECONDARY_SLOT, bs);
...
#    endif
        ...
        BOOT_IMG(state, BOOT_PRIMARY_SLOT).sectors   = primary_slot_sectors[image_index];
        BOOT_IMG(state, BOOT_SECONDARY_SLOT).sectors = secondary_slot_sectors[image_index];
#    if MCUBOOT_SWAP_USING_SCRATCH
...
#    endif
        ...
        if (BOOT_IS_UPGRADE(BOOT_SWAP_TYPE(state))) { has_upgrade = true; }
    }
...
#    endif /* (BOOT_IMAGE_NUMBER > 1) */
        ...
        switch (BOOT_SWAP_TYPE(state)) {
        case BOOT_SWAP_TYPE_NONE: break;
...
#    endif /* !MCUBOOT_OVERWRITE_ONLY */
        ...
        default: BOOT_SWAP_TYPE(state) = BOOT_SWAP_TYPE_PANIC;
        }
...
#    ifdef MCUBOOT_VALIDATE_PRIMARY_SLOT
        ...
        if (fih_not_eq(fih_rc, FIH_SUCCESS)) { goto out; }
#    else
        ...
        if (BOOT_IMG(state, BOOT_PRIMARY_SLOT).hdr.ih_magic != IMAGE_MAGIC) {
            BOOT_LOG_ERR("bad image magic 0x%lx; Image=%u",
                         (unsigned long)&boot_img_hdr(state, BOOT_PRIMARY_SLOT)->ih_magic,
                         BOOT_CURR_IMG(state));
            rc = BOOT_EBADIMAGE;
            goto out;
        }
...
#    ifdef MCUBOOT_HW_ROLLBACK_PROT
        ...
        if (BOOT_SWAP_TYPE(state) == BOOT_SWAP_TYPE_NONE) {
            rc = boot_update_security_counter(
                BOOT_CURR_IMG(state), BOOT_PRIMARY_SLOT, boot_img_hdr(state, BOOT_PRIMARY_SLOT));
            if (rc != 0) {
...
#    ifdef MCUBOOT_MEASURED_BOOT
        ...
        if (rc != 0) {
            BOOT_LOG_ERR("Failed to add Image %u data to shared memory area", BOOT_CURR_IMG(state));
            goto out;
        }
...
#    endif
    ...
    if (rc) { fih_rc = fih_int_encode(rc); }
...
fih_int split_go(int loader_slot, int split_slot, void** entry)
{
    ...
    if (sectors == NULL) { FIH_RET(FIH_FAILURE); }
    ...
    rc              = flash_area_open(loader_flash_id, &BOOT_IMG_AREA(&boot_data, loader_slot));
    assert(rc == 0);
    split_flash_id = flash_area_id_from_image_slot(split_slot);
    rc             = flash_area_open(split_flash_id, &BOOT_IMG_AREA(&boot_data, split_slot));
    assert(rc == 0);
 ...
 */
static uint32_t boot_get_slot_usage(struct boot_loader_state* state, uint8_t slot_usage[],
                                    uint32_t slot_cnt)
{
    ...
    for (slot = 0; slot < slot_cnt; slot++) {
        ...
        if (boot_is_header_valid(hdr, BOOT_IMG_AREA(state, slot))) {
            ...
            BOOT_LOG_INF("%s slot: Image not found",
                         (slot == BOOT_PRIMARY_SLOT) ? "Primary" : "Secondary");
        }
    }
 ...
 * @return          0 on success; nonzero on failure.
 */
static int boot_select_or_erase(struct boot_swap_state* state, uint32_t slot)
{
    const struct flash_area* fap;
    int                      fa_id;
    int                      rc;
    ...
    if (state->magic != BOOT_MAGIC_GOOD ||
        (state->copy_done == BOOT_FLAG_SET && state->image_ok != BOOT_FLAG_SET)) {
        /*
 ...
 * @return                0 on success; nonzero on failure.
 */
static int boot_verify_ram_load_address(uint32_t img_dst, uint32_t img_sz)
{
    uint32_t img_end_addr;

    if (img_dst < IMAGE_EXECUTABLE_RAM_START) { return BOOT_EBADIMAGE; }

    if (!boot_u32_safe_add(&img_end_addr, img_dst, img_sz)) { return BOOT_EBADIMAGE; }

    if (img_end_addr > (IMAGE_EXECUTABLE_RAM_START + IMAGE_EXECUTABLE_RAM_SIZE)) {
 ...
 * @return          0 on success; nonzero on failure.
 */
static int boot_copy_image_to_sram(int slot, uint32_t img_dst, uint32_t img_sz)
{
    int                      rc;
    ...
    if (rc != 0) { BOOT_LOG_INF("Error whilst copying image from Flash to SRAM: %d", rc); }
 ...
 * @return          0 on success; nonzero on failure.
 */
static int boot_load_image_to_sram(struct boot_loader_state* state, uint32_t slot,
                                   struct image_header* hdr, uint32_t* img_dst, uint32_t* img_sz)
{
    int rc;
    ...
    if (hdr->ih_flags & IMAGE_F_RAM_LOAD) {
        ...
        *img_dst = hdr->ih_load_addr;
        ...
        if (rc != 0) { BOOT_LOG_INF("RAM loading to 0x%x is failed.", *img_dst); }
        else {
 ...
 * @return          0 on success; nonzero on failure.
 */
static inline int boot_remove_image_from_sram(uint32_t img_dst, uint32_t img_sz)
{
...
#    endif /* MCUBOOT_DIRECT_XIP_REVERT */
    ...
    img_cnt = boot_get_slot_usage(state, slot_usage, sizeof(slot_usage) / sizeof(slot_usage[0]));
            ...
            for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
                if (slot_usage[slot]) {
                    ...
                    if (selected_image_header != NULL) {
                        boot_version_rc =
                            boot_version_cmp(&hdr->ih_ver, &selected_image_header->ih_ver);
...
#    ifdef MCUBOOT_RAM_LOAD
             ...
             */
            rc = boot_load_image_to_sram(
                state, selected_slot, selected_image_header, &img_dst, &img_sz);
            if (rc != 0) {
...
#        endif
            rc = boot_update_security_counter(0, selected_slot, selected_image_header);
            if (rc != 0) {
                BOOT_LOG_ERR("Security counter update failed after image "
                             "validation.");
                goto out;
...
#    ifdef MCUBOOT_MEASURED_BOOT
        rc = boot_save_boot_status(0, selected_image_header, BOOT_IMG_AREA(state, selected_slot));
        if (rc != 0) { BOOT_LOG_ERR("Failed to add image data to shared area"); }
#    endif /* MCUBOOT_MEASURED_BOOT */

#    ifdef MCUBOOT_DATA_SHARING
        rc = boot_save_shared_data(selected_image_header, BOOT_IMG_AREA(state, selected_slot));
        if (rc != 0) { BOOT_LOG_ERR("Failed to add data to shared memory area."); }
#    endif /* MCUBOOT_DATA_SHARING */

        BOOT_LOG_INF("Booting image from the %s slot",
                     (selected_slot == BOOT_PRIMARY_SLOT) ? "primary" : "secondary");

        rsp->br_flash_dev_id = BOOT_IMG_AREA(state, selected_slot)->fa_device_id;
...
fih_int context_return_highest_version(struct boot_loader_state* state, struct boot_rsp* rsp)
{
    struct image_header* hdr                   = NULL;
    struct image_header* selected_image_header = NULL;
    uint8_t              slot_usage[BOOT_NUM_SLOTS];
        ...
        for (i = 0; i < img_cnt; i++) {
            ...
            for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
                if (slot_usage[slot]) {
                    hdr = boot_img_hdr(state, slot);
                    if (selected_image_header != NULL) {
                        rc = boot_version_cmp(&hdr->ih_ver, &selected_image_header->ih_ver);
                        if (rc < 1) {
                            /* The version of the image being examined wasn't
                             * greater than the currently selected image's
                             * version.
...
int boot_compare_version(struct image_version* ver1, volatile const struct image_version* ver2)
{
    if (ver1->iv_major > ver2->iv_major) { return 1; }
    if (ver1->iv_major < ver2->iv_major) { return -1; }
    /* The major version numbers are equal, continue comparison. */
