...
#    if defined(MCUBOOT_USE_CC310) || defined(MCUBOOT_USE_TI_CRYPTO) || \
        defined(MCUBOOT_USE_MSP_CRYPTO)
#        define NUM_ECC_BYTES (256 / 8)
#    endif
#    if defined(MCUBOOT_USE_TINYCRYPT) || defined(MCUBOOT_USE_CC310) || \
        defined(MCUBOOT_USE_TI_CRYPTO) || defined(MCUBOOT_USE_MSP_CRYPTO)
...
static int bootutil_import_key(uint8_t** cp, uint8_t* end)
{
    ...
    /* ECPoint (RFC5480) */
    if (mbedtls_asn1_get_bitstring_null(cp, end, &len)) { return -6; }
    if (*cp + len != end) { return -7; }

    if (len != 2 * NUM_ECC_BYTES + 1) { return -8; }
    /* Is uncompressed? */
    if (*cp[0] != 0x04) { return -9; }
...
static int bootutil_read_bigint(uint8_t i[NUM_ECC_BYTES], uint8_t** cp, uint8_t* end)
{
    ...
    if (mbedtls_asn1_get_tag(cp, end, &len, MBEDTLS_ASN1_INTEGER)) { return -3; }

    if (len >= NUM_ECC_BYTES) { memcpy(i, *cp + len - NUM_ECC_BYTES, NUM_ECC_BYTES); }
    else {
...
int bootutil_verify_sig(uint8_t* hash, uint32_t hlen, uint8_t* sig, size_t slen, uint8_t key_id)
{
    int                         rc;
     ...
     */
    if (hlen != NUM_ECC_BYTES) { return -1; }
...
#    endif /* MCUBOOT_USE_TINYCRYPT || defined MCUBOOT_USE_CC310 || MCUBOOT_USE_TI_CRYPTO */
