...
int boot_add_data_to_shared_area(uint8_t major_type, uint16_t minor_type, size_t size,
                                 const uint8_t* data)
{
    ...
    if (data == NULL) { return SHARED_MEMORY_GEN_ERROR; }
    ...
    if (!boot_u16_safe_add(
            &boot_data_size, boot_data->header.tlv_tot_len, SHARED_DATA_ENTRY_SIZE(size))) {
        return SHARED_MEMORY_GEN_ERROR;
    }

    /* Verify overflow of shared area */
    if (boot_data_size > MCUBOOT_SHARED_DATA_SIZE) { return SHARED_MEMORY_OVERFLOW; }
...
/* See in boot_record.h */
int boot_save_boot_status(uint8_t sw_module, const struct image_header* hdr,
                          const struct flash_area* fap)
{
    ...
    while (true) {
        ...
        if (type == IMAGE_TLV_BOOT_RECORD) {
            ...
            if (len > sizeof(image_hash)) { return -1; }
    ...
    if (!boot_record_found || !hash_found) { return -1; }
    ...
    rc        = boot_add_data_to_shared_area(TLV_MAJOR_IAS, ias_minor, record_len, buf);
    if (rc != SHARED_MEMORY_OK) { return rc; }
