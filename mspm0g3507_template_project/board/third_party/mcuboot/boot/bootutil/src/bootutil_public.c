...
#define BOOT_MAGIC_ARR_SZ (sizeof boot_img_magic / sizeof boot_img_magic[0])
...
#define BOOT_SWAP_TABLES_COUNT (sizeof boot_swap_tables / sizeof boot_swap_tables[0])

static int boot_magic_decode(const uint32_t* magic)
{
    if (memcmp(magic, boot_img_magic, BOOT_MAGIC_SZ) == 0) { return BOOT_MAGIC_GOOD; }
    return BOOT_MAGIC_BAD;
}

static int boot_flag_decode(uint8_t flag)
{
    if (flag != BOOT_FLAG_SET) { return BOOT_FLAG_BAD; }
...
static inline uint32_t boot_swap_size_off(const struct flash_area* fap)
{
...
{
    switch (tbl_val) {
    case BOOT_MAGIC_ANY: return 1;

    case BOOT_MAGIC_NOTGOOD: return val != BOOT_MAGIC_GOOD;

    default: return tbl_val == val;
    }
}

#ifdef MCUBOOT_ENC_IMAGES
static inline uint32_t boot_enc_key_off(const struct flash_area* fap, uint8_t slot)
{
#    if MCUBOOT_SWAP_SAVE_ENCTLV
    return boot_swap_size_off(fap) -
           ((slot + 1) * ((((BOOT_ENC_TLV_SIZE - 1) / BOOT_MAX_ALIGN) + 1) * BOOT_MAX_ALIGN));
#    else
...
bool bootutil_buffer_is_erased(const struct flash_area* area, const void* buffer, size_t len)
{
    ...
    for (i = 0, u8b = (uint8_t*)buffer; i < len; i++) {
        if (u8b[i] != erased_val) { return false; }
    }
...
static int boot_read_flag(const struct flash_area* fap, uint8_t* flag, uint32_t off)
{
    ...
    if (bootutil_buffer_is_erased(fap, flag, sizeof *flag)) { *flag = BOOT_FLAG_UNSET; }
    else {
...
int boot_read_swap_state(const struct flash_area* fap, struct boot_swap_state* state)
{
    ...
    if (rc < 0) { return BOOT_EFLASH; }
    if (bootutil_buffer_is_erased(fap, magic, BOOT_MAGIC_SZ)) { state->magic = BOOT_MAGIC_UNSET; }
    else {
 ...
 */
static int boot_write_trailer(const struct flash_area* fap, uint32_t off, const uint8_t* inbuf,
                              uint8_t inlen)
{
    ...
    if (inlen > BOOT_MAX_ALIGN || align > BOOT_MAX_ALIGN) { return -1; }
    erased_val = flash_area_erased_val(fap);
    if (align < inlen) { align = inlen; }
...
static int boot_write_trailer_flag(const struct flash_area* fap, uint32_t off, uint8_t flag_val)
{
...
int boot_swap_type_multi(int image_index)
{
    ...
    if (rc) { return BOOT_SWAP_TYPE_PANIC; }
...
int boot_set_pending(int permanent)
{
    ...
    rc = boot_read_swap_state_by_id(FLASH_AREA_IMAGE_SECONDARY(0), &state_secondary_slot);
    if (rc != 0) { return rc; }
    ...
    case BOOT_MAGIC_UNSET:
        ...
        if (rc != 0) { rc = BOOT_EFLASH; }
        else {
            rc = boot_write_magic(fap);
        }

        if (rc == 0 && permanent) { rc = boot_write_image_ok(fap); }

        if (rc == 0) {
            if (permanent) { swap_type = BOOT_SWAP_TYPE_PERM; }
            else {
                swap_type = BOOT_SWAP_TYPE_TEST;
            }
    ...
    default: assert(0); return BOOT_EBADIMAGE;
    }
}
...
int boot_set_confirmed(void)
{
    ...
    rc = boot_read_swap_state_by_id(FLASH_AREA_IMAGE_PRIMARY(0), &state_primary_slot);
    if (rc != 0) { return rc; }
