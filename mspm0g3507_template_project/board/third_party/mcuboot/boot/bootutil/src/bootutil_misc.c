...
inline fih_int boot_fih_memequal(const void* s1, const void* s2, size_t n)
{
...
fih_int boot_fih_memequal(const void* s1, const void* s2, size_t n)
{
    ...
    for (i = 0; i < n; i++) {
        if (s1_p[i] != s2_p[i]) { goto out; }
    }
    if (i == n) { ret = FIH_SUCCESS; }
...
int boot_status_entries(int image_index, const struct flash_area* fap)
{
    ...
    if (fap->fa_id == FLASH_AREA_IMAGE_SCRATCH) { return BOOT_STATUS_STATE_COUNT; }
    else
#endif
 ...
 */
static int boot_find_status(int image_index, const struct flash_area** fap)
{
    ...
    for (i = 0; i < sizeof(areas) / sizeof(areas[0]); i++) {
        ...
        if (memcmp(magic, boot_img_magic, BOOT_MAGIC_SZ) == 0) { return 0; }
...
#    if MCUBOOT_SWAP_SAVE_ENCTLV
        ...
        if (rc == 0) {
            for (i = 0; i < BOOT_ENC_TLV_ALIGN_SIZE; i++) {
                if (bs->enctlv[slot][i] != 0xff) { break; }
            }
