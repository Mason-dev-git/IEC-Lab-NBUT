...
#endif
#if defined(MCUBOOT_ENC_IMAGES) || defined(MCUBOOT_SIGN_RSA) || defined(MCUBOOT_SIGN_EC) || \
 ...
 */
static int bootutil_img_hash(struct enc_key_data* enc_state, int image_index,
...
#if (BOOT_IMAGE_NUMBER == 1) || !defined(MCUBOOT_ENC_IMAGES) || defined(MCUBOOT_RAM_LOAD)
    (void)enc_state;
    (void)image_index;
    (void)hdr_size;
    (void)blk_off;
...
#    endif
    ...
    if (MUST_DECRYPT(fap, image_index, hdr) && !boot_enc_valid(enc_state, image_index, fap)) {
        return -1;
    }
#endif
     ...
     * the loader image */
    if (seed && (seed_len > 0)) { bootutil_sha256_update(&sha256_ctx, seed, seed_len); }
    ...
    for (off = 0; off < size; off += blk_sz) {
        blk_sz = size - off;
        if (blk_sz > tmp_buf_sz) { blk_sz = tmp_buf_sz; }
#    ifdef MCUBOOT_ENC_IMAGES
...
#    ifdef MCUBOOT_ENC_IMAGES
        if (MUST_DECRYPT(fap, image_index, hdr)) {
            ...
            if (off >= hdr_size && off < tlv_off) {
                ...
                boot_encrypt(enc_state, image_index, fap, off - hdr_size, blk_sz, blk_off, tmp_buf);
            }
        }
#    endif
 ...
 */
#if (defined(MCUBOOT_SIGN_RSA) + defined(MCUBOOT_SIGN_EC) + defined(MCUBOOT_SIGN_EC256) + \
...
static int          bootutil_find_key(uint8_t image_index, uint8_t* key, uint16_t key_len)
{
    bootutil_sha256_context sha256_ctx;
    uint8_t                 hash[32];
    uint8_t                 key_hash[32];
 ...
 */
int32_t bootutil_get_img_security_cnt(struct image_header* hdr, const struct flash_area* fap,
                                      uint32_t* img_security_cnt)
{
    ...
    if ((hdr == NULL) || (fap == NULL) || (img_security_cnt == NULL)) {
        /* Invalid parameter. */
        return BOOT_EBADARGS;
    }

    /* The security counter TLV is in the protected part of the TLV area. */
    if (hdr->ih_protect_tlv_size == 0) { return BOOT_EBADIMAGE; }
...
fih_int bootutil_img_validate(struct enc_key_data* enc_state, int image_index,
                              struct image_header* hdr, const struct flash_area* fap,
                              uint8_t* tmp_buf, uint32_t tmp_buf_sz, uint8_t* seed, int seed_len,
                              uint8_t* out_hash)
{
    uint32_t off;
    uint16_t len;
...
#    endif
    ...
    rc = bootutil_img_hash(
        enc_state, image_index, hdr, fap, tmp_buf, tmp_buf_sz, hash, seed, seed_len);
    if (rc) { goto out; }
...
#    endif /* !MCUBOOT_HW_KEY */
             ...
             */
            fih_rc = fih_int_encode_zero_equality(img_security_cnt < fih_int_decode(security_cnt));
            if (fih_not_eq(fih_rc, FIH_SUCCESS)) { goto out; }
    ...
    if (rc) { goto out; }
#ifdef EXPECTED_SIG_TLV
    fih_rc = fih_int_encode_zero_equality(fih_not_eq(valid_signature, FIH_SUCCESS));
#endif
#ifdef MCUBOOT_HW_ROLLBACK_PROT
