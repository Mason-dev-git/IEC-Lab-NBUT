...
uint16_t ad_now[8]={0};//adc采样当前值
uint16_t ad_max[8]={0};//adc采样最大值
uint16_t ad_min[8]={4095, 4095, 4095, 4095, 4095, 4095,4095, 4095};//adc采样最小值
uint16_t ad_avr[8]={2048, 2048, 2048, 2048, 2048, 2048,2048, 2048};//adc采样平均值
int32_t *error_avr=new int32_t[8]; //误差平均
...
{
 HAL_ADC_Start_DMA(&hadc1, (uint32_t *)ad_now, 8);//参数：句柄；地址；长度
}
...
{ 
	...
	{
		yaw_error=-1.0*(yaw_compensation-yaw);  //不要改，这里的bug很神奇，改了跑不了
		return yaw_error;
	}
	else if(yaw_error<=-270&&yaw_error>=-360)
	{
		yaw_error=yaw+360-yaw_compensation;
		return yaw_error;
	}
	else
	{
...
{
  if(ad_now[ad_value-2]<=1200)  //遇白线停止转向
  {
    HAL_Delay(2);
    if(ad_now[ad_value-2]<=1200)
    {
      return 1;
    }
  }
...
int32_t PatrolLine::ad_compensation()
{
  ...
  else if(ad_now[7]<=1500)
  {
    return 100;
  }
  else if(ad_now[1]<=1500)
  {
    return -75;
  }
  else if(ad_now[6]<=1500)
  {
    return 75;
  }
  else if(ad_now[2]<=1500)
  {
    return -50;
  }
  else if(ad_now[5]<=1500)
  {
    return 50;
  }
  else
